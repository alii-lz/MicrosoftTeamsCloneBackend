...
export function userStatsV1 (token: string) {
	...
	for (let i = 0; i < numChannels; i++) {
		...
		const user = data.channels[i].allMembers.find((user: User) => user.uId === id);
		if (user) {
			numChannelsJoined++;
	...
	const numMessagesSent = numMessagesByUserInDms + numMessagesByUserInChannels;

	const stats = {
		channelsJoined: [{numChannelsJoined: numChannelsJoined, timeStamp: currTime}],
		dmsJoined: [{numDmsJoined: numDmsJoined, timeStamp: currTime}],
		messagesSent: [{numMessagesSent: numMessagesSent, timeStamp: currTime}],
		involvementRate: (numChannelsJoined + numDmsJoined + numMessagesSent) / (numChannels + numDms + numMessages)
	}
...
export function usersStatsV1 (token: string) {
	...
	const stats = {
		channelsExist: [{numChannelsExist: numChannelsExist, timeStamp: currTime}],
		dmsExist: [{numDmsExist: numDmsExist, timeStamp: currTime}],
		messagesExist: [{numMessagesExist: numMessagesExist, timeStamp: currTime}],
