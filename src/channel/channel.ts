...
export function channelJoinV3(
    ...
    if (
        channelPointer.isPublic === false &&
        authUserId !== dataStore.users[0].uId
    ) {
...
export function channelInviteV1(
    ...
    if (data.indivNotification.length === 0) {
        ...
        let notiIndex = 0;
        while (
            notiIndex < data.indivNotification.length &&
        ...
        // User already has notifications.
            ...
            data.indivNotification[notiIndex].notification.unshift(
                newNotification
            );
        }
    }
...
export function channelMessagesV1(
    ...
    if (data.channels[channelIndex].messages === undefined) {
        ...
        if (start > 0) {
            ...
            throw HTTPError(
                400,
                "Message number entered exceeds the number of messages in this channel."
            );
        }
    }
    if (nullArray === false) {
        ...
        for (
            let k = 0;
            k < data.channels[channelIndex].allMembers.length;
            k++
        ) {
        ...
        if (start + 50 < numberOfMessages) {
            ...
            let inc = start;
            while (inc < end) {
                messageArrayTemp.push(
                    data.channels[channelIndex].messages[inc]
                );
                inc++;
            }
        } else {
            ...
            const lastMessageIndex =
                data.channels[channelIndex].messages.length;
