...
export function channelRemoveOwnerV2(
    ...
    // }
    if (
        data.channels[channelIndex].owners.length === 1 &&
        data.channels[channelIndex].owners[0].uId === uId
    ) {
    ...
    if (
        data.channels
            ...
            .owners.find((j: { uId: number }) => j.uId === id) === undefined
    ) {
        throw HTTPError(
            403,
    ...
    let j = 0;
    while (
        j < data.channels[channelIndex].owners.length &&
        data.channels[channelIndex].owners[j].uId !== uId
    ) {
        j++;
    }
    ...
    while (
        ownerIndex < data.channels[channelIndex].owners.length &&
        data.channels[channelIndex].owners[ownerIndex].uId !== uId
    ) {
        ownerIndex++;
    }
