 ...
 */
export function messageSendV2(
    token: string,
    channelId: number,
    message: string
): messageIDReturn | error {
    const data = getData();
    ...
    while (
        channelIndex2 < totalChannelCount &&
        data.channels[channelIndex2].channelId !== channelId
    ) {
        channelIndex2++;
    }
    ...
    while (
        channelUserIndex < memberCount &&
        data.channels[channelIndex2].allMembers[channelUserIndex].uId !== uId
    ) {
        channelUserIndex++;
    }
...
export function messageEditV2(
    ...
    if (
        data.messageDetails[i].uId !== uId &&
        data.users[userIndex].globalOwner === false
    ) {
        throw HTTPError(
            403,
            "User is not a global owner. Cannot edit message."
        );
    ...
    while (
        ...
        data.messageDetails[i].channelId
    ) {
        channelIndex++;
    }
    ...
    while (
        data.channels[channelIndex].messages[messageIndexInChannel]
            .messageId !== messageId
    ) {
        messageIndexInChannel++;
    }
    // Actually replacing the message.
    data.channels[channelIndex].messages[messageIndexInChannel].message =
        message;
    // return
    setData(data);
    return {};
...
export function messageRemoveV2(token: string, messageId: number): any | error {
    ...
    if (
        ...
        throw HTTPError(
            403,
            "User is not a global owner. Cannot remove message."
        );
...
export function messageSenddmV2(token: string, dmId: number, message: string) {
    ...
    let dmUserIndex = 0;
    while (
        dmUserIndex < data.dm[dmIndex].members.length &&
        data.dm[dmIndex].members[dmUserIndex] !== uId
    ) {
        dmUserIndex++;
    }
