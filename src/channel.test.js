...
describe('channelDetailsV1(authUserId, channelId): Invalid Inputs', () => {
  ...
  test('Missing ChannelId1', () => {
    expect(channelDetailsV1(AuthUserId1.uId, '')).toStrictEqual(ERROR);
  });
  test('Invalid AuthUserId (not in storeData)', () => {
    expect(channelDetailsV1(111, ChannelId1.channelId)).toStrictEqual(ERROR);
  });
  test('Invalid ChannelId (not in storeData)', () => {
    expect(channelDetailsV1(AuthUserId1.uId, 1010)).toStrictEqual(ERROR);
  });
  test('AuthUserId is not a member of the channel', () => {
    expect(channelDetailsV1(AuthUserId2.uId, ChannelId3.channelId)).toStrictEqual(ERROR);
  });
  ...
  describe('channelDetailsV1: Correct Results', () => {
    ...
    beforeEach(() => {
      ...
      AuthUserId2 = authRegisterV1('ron.weasley@gmail.com', 'flying car', 'Ron', 'Weasley');
      AuthUserId3 = authRegisterV1('hermione.granger@gmail.com', 'reading', 'Hermione', 'Granger');
      ChannelId1 = channelsCreateV1(AuthUserId1.authUserId, 'Quidditch League', true);
      ChannelId2 = channelsCreateV1(AuthUserId2.authUserId, 'The Weasleys', false);
      ChannelId3 = channelsCreateV1(AuthUserId3.authUserId, 'Hermione Fan Club', true);
    });
    test('channelDetailsV1(authUserId, channelId): Harry Potter', () => {
      expect(channelDetailsV1(AuthUserId1.authUserId, ChannelId1.channelId)).toStrictEqual({
    ...
    test('channelDetailsV1(authUserId, channelId): Hermione Granger', () => {
      expect(channelDetailsV1(AuthUserId3.authUserId, ChannelId3.channelId)).toStrictEqual({
...
describe('channelJoinV1(authUserId, channelId): Invalid Inputs', () => {
  ...
  test('Missing ChannelId1', () => {
    expect(channelJoinV1(AuthUserId1.authUserId, '')).toStrictEqual(ERROR);
  });
  test('Invalid AuthUserId (not in storeData)', () => {
    expect(channelJoinV1(10101010, ChannelId1.channelId)).toStrictEqual(ERROR);
  });
  test('Invalid ChannelId (not in storeData)', () => {
    expect(channelJoinV1(AuthUserId1.authUserId, 1010)).toStrictEqual(ERROR);
  });
  test('AuthUserId cannot join the channel because it is a private channel', () => {
    expect(channelJoinV1(AuthUserId3.authUserId, ChannelId2.channelId)).toStrictEqual(ERROR);
  });
  test('AuthUserId is already a member of the channel', () => {
    expect(channelJoinV1(AuthUserId1.authUserId, ChannelId1.channelId)).toStrictEqual(ERROR);
  });
});
...
describe('channelJoinV1(authUserId, channelId): Correct Results', () => {
  ...
  test('channelDetailsV1(authUserId, channelId): Harry Potter', () => {
    channelJoinV1(AuthUserId3.authUserId, ChannelId1.channelId);
    expect(channelDetailsV1(AuthUserId1.authUserId, ChannelId1.channelId)).toStrictEqual({
      name: 'Quidditch League',
      isPublic: true,
      ownerMembers: [
  ...
  test('channelDetailsV1(authUserId, channelId): Ron Weasley', () => {
    channelJoinV1(AuthUserId1.authUserId, ChannelId2.channelId);
    expect(channelDetailsV1(AuthUserId1.authUserId, ChannelId2.channelId)).toStrictEqual({
      name: 'The Weasleys',
      isPublic: false,
      ownerMembers: [
        {
  ...
  test('channelDetailsV1(authUserId, channelId): Hermione Granger', () => {
    channelJoinV1(AuthUserId3.authUserId, ChannelId1.channelId);
    expect(channelDetailsV1(AuthUserId3.authUserId, ChannelId1.channelId)).toStrictEqual({
      name: 'Quidditch League',
      isPublic: true,
      ownerMembers: [
        {
...
describe('channelInviteV1', () => {
  ...
  test('Working case', () => {
    ...
    const channelId1 = channelsCreateV1(authId1.authUserId, 'Group_1', true);
  ...
  test('Invalid authId', () => {
    ClearV1();
    const authId = authRegisterV1('email@outlook.com', 'Password', 'Sam', 'R');
    const invitedUser = authRegisterV1('email2@outlook.com', 'Password2', 'Tom', 'S');
    const channelId1 = channelsCreateV1(authId.authUserId, 'Group_1', true);
...
describe('channelMessagesV1', () => {
  ...
  test('Invalid channelId', () => {
    ...
    const channelId1 = channelsCreateV1(user.authUserId, 'Group_1', true);
  ...
  test('authId not valid', () => {
    ...
    const authId1 = authRegisterV1('email@outlook.com', 'Password', 'Sam', 'R');
    const channelId1 = channelsCreateV1(authId1.authUserId, 'Group_1', true);
    const invalidId = authId1.authUserId + 10;
