...
function authRegisterV1(
    ...
    if (!ifDataStoreEmpty) {
        const existingHandle = data.users.find((user) =>
            user.handleStr.includes(handleStr)
        );
        if (existingHandle) {
            ...
            handleStr = !isNaN(parseInt(endCharacter))
                ? handleStr.replace(
                      /.$/,
                      (parseInt(endCharacter) + 1).toString()
                  )
                : `${handleStr}0`;
        }
    }
...
function authPasswordResetResetV1(resetCode: string, newPassword: string) {
    const data: Data = getData();
    const match = data.resetCodes.find(
        (item) => item.code === resetCode && item.valid === true
    );
    if (!match) {
