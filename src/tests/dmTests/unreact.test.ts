...
describe("/message/react/v1 failure tests", () => {
    beforeEach(() => {
        ...
        const AuthUserId2_return = requestAuthRegister(
        ...
        // Initalize dms
        const dm1_return = requestDmCreate(AuthUserId1.token, [
            AuthUserId2.authUserId,
        ]);
        dm1 = dm1_return.returnObj;
        const dm2_return = requestDmCreate(AuthUserId2.token, [
            AuthUserId1.authUserId,
        ]);
        dm2 = dm2_return.returnObj;

        // Initalize channelId
        const channelId1_return = requestChannelsCreateV3(
            AuthUserId1.token,
            "Quidditch League",
            true
        );
        ChannelId1 = channelId1_return.returnObj;
        const channelId2_return = requestChannelsCreateV3(
            AuthUserId2.token,
            "Hermione Fan Club",
            true
        );
        ...
        // Initalize messages
        const messageId1_return = requestMessageSendV2(
            AuthUserId1.token,
            ChannelId1.channelId,
            "Quidditch League"
        );
        messageId1 = messageId1_return.returnObj;
        const messageId2_return = requestMessageSendV2(
            AuthUserId2.token,
            ChannelId1.channelId,
            "Hermione Fan Club"
        );
...
describe("/message/react/v1 success case", () => {
    beforeEach(() => {
        ...
        // send Dm
        const dmMessageId1_return = requestMessageSendDmV2(
            AuthUserId1.token,
            dm1.dmId,
            "Holla!"
        );
        dmMessageId1 = dmMessageId1_return.returnObj;
        const dmMessageId2_return = requestMessageSendDmV2(
            AuthUserId2.token,
            dm2.dmId,
            "Holla!"
        );
