...
describe("Tests for /channels/create/v3", () => {
    ...
    test("success case", () => {
        const channel = requestChannelsCreateV3(
            user.returnObj.token,
            "validName",
            true
        );
        expect(channel.status).toBe(OK);
        expect(channel.returnObj).toStrictEqual({
    ...
    test("name length more than 20", () => {
        try {
            const channel = requestChannelsCreateV3(
                user.returnObj.token,
                "THISNAMEISTOOLONGGGGGGGGGG",
                true
            );
            expect(channel.returnObj.error).toEqual({
...
describe("Tests for /channels/list/v3", () => {
    ...
    test("invalid token", () => {
        try {
            const result = requestChannelsCreateV3(
                user.returnObj.token + "a",
                "validName",
                true
            );
