...
describe("Tests for /channel/addowner/v2", () => {
    ...
    beforeEach(() => {
        ...
        requestChannelInviteV3(
            ...
            userToAdd.returnObj.authUserId
        );
    });

    test("success case", () => {
        const result = requestChannelAddOwnerV2(
            user.returnObj.token,
            channel.returnObj.channelId,
            userToAdd.returnObj.authUserId
        );
    ...
    test("channelId does not refer to a valid channel", () => {
        try {
            const result = requestChannelAddOwnerV2(
                user.returnObj.token,
                channel.returnObj.channelId + 100,
                userToAdd.returnObj.authUserId
            );
    ...
    test("uId does not refer to a valid user", () => {
        try {
            const result = requestChannelAddOwnerV2(
                user.returnObj.token,
                channel.returnObj.channelId,
                userToAdd.returnObj.authUserId + 20
            );
