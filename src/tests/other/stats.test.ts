...
describe("Tests for /user/stats/v1", () => {
    ...
    test("no messages/dms success case", () => {
        ...
        expect(
            result.returnObj.userStats.channelsJoined[0].numChannelsJoined
        ).toBe(1);
        expect(
            result.returnObj.userStats.channelsJoined[0].timeStamp
        ).toBeGreaterThan(1000000000);
        expect(result.returnObj.userStats.dmsJoined[0].numDmsJoined).toBe(0);
        expect(
            result.returnObj.userStats.dmsJoined[0].timeStamp
        ).toBeGreaterThan(1000000000);
        expect(result.returnObj.userStats.messagesSent[0].numMessagesSent).toBe(
            0
        );
        expect(
            result.returnObj.userStats.messagesSent[0].timeStamp
        ).toBeGreaterThan(1000000000);
    ...
    test("dms success case", () => {
        const userInDm = requestAuthRegister(
            "temp@gmail.com",
            "temp",
            "temp",
            "temp"
    ...
    test("0 involement case", () => {
        ...
        const tempUser = requestAuthRegister(
            "abcd@gmail.com",
            "temp1",
            "temp1",
            "temp1"
...
describe("Tests for /users/stats/v1", () => {
    ...
    test("only channels exist success case", () => {
        ...
        expect(result.status).toBe(OK);
        expect(
            result.returnObj.workspaceStats.channelsExist[0].numChannelsExist
        ).toBe(1);
        expect(
            result.returnObj.workspaceStats.channelsExist[0].timeStamp
        ).toBeGreaterThan(1000000000);
        expect(result.returnObj.workspaceStats.dmsExist[0].numDmsExist).toBe(0);
        expect(
            result.returnObj.workspaceStats.dmsExist[0].timeStamp
        ).toBeGreaterThan(1000000000);
        expect(
            result.returnObj.workspaceStats.messagesExist[0].numMessagesExist
        ).toBe(0);
        expect(
            result.returnObj.workspaceStats.messagesExist[0].timeStamp
        ).toBeGreaterThan(1000000000);
