...
export function notificationsGetV1(token: string): {notifications: []} {
  ...
  while (dataNotiUserIndex < data.indivNotification.length && data.indivNotification[dataNotiUserIndex].userId !== uId) {
    dataNotiUserIndex++;
  }
  ...
  for (let i = 0; i < 20; i++) {
    if (data.indivNotification[dataNotiUserIndex].notification[i] !== undefined) {
      notiDisplay[i] = data.indivNotification[dataNotiUserIndex].notification[i];
